...
class ParserARC():
    ...
    def procesarArchivo(self, registrosConErrores, mis_filepathname, ultimoRegistro):
        ...
        for row in dataReader:
            ...
            registro.outtemp = self.parseFloat(row[1])
            registro.windchill = self.parseFloat(row[2])
            registro.heatindex = self.parseFloat(row[3])
            registro.outhumidity = self.parseFloat(row[4])
            registro.dewpoint = self.parseFloat(row[5])
            registro.windspeed = self.parseFloat(row[6])
            registro.windgust = self.parseFloat(row[7])
            registro.winddir = self.parseFloat(row[8])
            registro.rain = self.parseFloat(row[9])
            registro.barometer = self.parseFloat(row[10])
            registro.soiltemp1 = self.parseFloat(row[11])
            registro.et = self.parseFloat(row[12])
            registro.uv = self.parseFloat(row[13])
